package everyday.zxz.com.everyday.widget;
...
import everyday.zxz.com.everyday.R;
import everyday.zxz.com.everyday.utils.DensityUtil;
...
public class MarqueeView extends View {
    ...
    private void inflateAttributes(AttributeSet attrs) {
        ...
        mTextSize = ta.getDimension(R.styleable.Lrc_textSizea, DensityUtil.dip2px(getContext(), 12));
        mRows = ta.getInteger(R.styleable.Lrc_rows, 1);
        mDividerHeight = ta.getDimension(R.styleable.Lrc_dividerHeight, 10);

        int normalTextColor = ta.getColor(R.styleable.Lrc_normalTextColor, 0xff00ffde);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLrcs.isEmpty() || mTimes.isEmpty()) {
            canvas.drawText(DEFAULT_TEXT, (mViewWidth - mCurrentPaint.measureText(DEFAULT_TEXT)) / 2, centerY,
                    mCurrentPaint);
            return;
        }
        ...
        if (currentX == 0) {
            int timeSpace = (int) (mTimes.get(mCurrentLine + 1) - mTimes.get(mCurrentLine)) - 1200;
            int alreadyTime = (int) (currentTime - mTimes.get(mCurrentLine));
            int mMaxScrollX = (int) (mCurrentPaint.measureText(currentLrc) - mViewWidth);
            Log.e(TAG, mCurrentLine + " timeSpace: " + timeSpace);
            Log.e(TAG, "currentTime: " + currentTime);
            Log.e(TAG, "alreadyTime: " + alreadyTime);
            mOffsetX = (int) ((alreadyTime * 1.0 / (timeSpace) * mMaxScrollX));
            mOffsetX = mOffsetX < 0 ? 0 : mOffsetX;
            mOffsetX = mOffsetX >= mMaxScrollX ? mMaxScrollX : mOffsetX;
            Log.e(TAG, mOffsetX + "mOffsetX: " + mMaxScrollX);
        } else {
            mOffsetX = 0;
        }
        // 画当前行
        canvas.drawText(currentLrc, currentX - mOffsetX, centerY - mOffsetY, mCurrentPaint);
    ...
    private Long parseTime(String time) {
        ...
        long milInt = Long.parseLong(sec[1].replaceAll("\\D+", "").replaceAll("\r", "").replaceAll("\n", "").trim());
    ...
    public synchronized void changeCurrent(long time) {
        currentTime = time;
        time += +SCROLL_TIME + 200;
        if (mTimes == null || mTimes.size() == 0)
            return;
        if ((mNextTime > time && time > mCurrentTime) || time < mTimes.get(1)) {
            postInvalidate();
            return;
        }
        // 每次进来都遍历存放的时间
        int timeSize = mTimes.size();
        Log.e(TAG, "changeCurrent: " + time);
        for (int i = 0; i < timeSize; i++) {
            ...
            if (mNextTime == mTimes.get(timeSize - 1)) {
                ...
                mScroller.startScroll(0, 0, timeSize, mMaxScroll, SCROLL_TIME);
            ...
            if (mTimes.get(i) > time) {
                Log.e(TAG, i + "changeCurrent: Time " + mTimes.get(i));
                if (i == 0 && mCurrentLine == 0)
                    return;
                int cur = i == 0 ? 0 : i - 1;
                Log.e(TAG, i + "changeCurrent: " + cur);
                mCurrentTime = mTimes.get(cur);
                mNextTime = mTimes.get(cur + 1);
                mOffsetX = 0;
                mScroller.abortAnimation();
                mScroller.startScroll(0, 0, cur + 1, mMaxScroll, SCROLL_TIME);
                postInvalidate();
                return;
            }
        }
